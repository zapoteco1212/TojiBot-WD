...
global.db.data.settings[botDir].isModBot = true
...
console.log(chalk.cyanBright(`[ MOD-BOT ] Conectado: ${sock.userId}`))
}
...
if (intentos < 5) {
console.log(chalk.yellow(`[ MOD-BOT ] ${botId} Cerrado (${reason}) intento ${intentos}/5 → Reintentando...`))
setTimeout(() => startModBot(m, client, caption, isCode, phone, chatId, {}, isCommand), 3000)
} else {
console.log(chalk.red(`[ MOD-BOT ] ${botId} Falló tras 5 intentos. Eliminando sesión.`))
...
await loadEvents(sock).catch(e => console.log(chalk.gray(`[ MOD-EVENTS ] → ${e}`)))
